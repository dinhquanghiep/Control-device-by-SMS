...
{
	// Set pin modes
	//HIEP ==> Do not need to control turn on module SIM
	...
	//HIEP ==> Module SIM always turn on
	// for (cont=0; cont<3; cont++) {
	//   //if (AT_RESP_ERR_NO_RESP == SendATCmdWaitResp(str_at, 500, 100, str_ok, 5) && !turnedON) {   //check power
	//   if (!turnedON && AT_RESP_ERR_NO_RESP == SendATCmdWaitResp(str_at, 500, 100, str_ok, 5)) {    //check power
	//     // there is no response => turn on the module
	//     #ifdef DEBUG_ON
	//       Serial.println(F("DB:NO RESP. Try to turn ON module...."));
	//     #endif
	...
	// if (!turnedON){
	//   #ifdef DEBUG_ON
	//     Serial.println(F("Can not turnON Module Sim. Pls. check power!"));
	//   #endif
	//     SetCommLineStatus(CLS_FREE);
	//     return -1;
	// }
	//HIEP
			//Kiem tra Module sim co duoc dat dung baud la 9600
	...
	//truong hop Module sim khong duoc dat o baud 9600 ==>> co tra loi nhung ko nhan dc OK
	// if (!turnedON && AT_RESP_ERR_DIF_RESP == SendATCmdWaitResp(str_at, 500, 100, str_ok, 5)) {    //check OK
	if (norepOK && AT_RESP_ERR_DIF_RESP == SendATCmdWaitResp(str_at, 500, 100, str_ok, 5)) {    //check OK
		#ifdef DEBUG_ON
		...
		for (int i=0; i<8; i++) {
			...
			if (AT_RESP_OK == SendATCmdWaitResp(str_at, 500, 100, str_ok, 5)) {
				...
				#endif
				// Neu da bik duoc toc do baud that thi chuyen ve toc do bau do minh cai dat
				_cell.print("AT+IPR=");
				...
				_cell.begin(baud_rate);
				delay(100);
				//Kiem tra lai lan nua voi baud rate moi_do minh thiet dat
				if (AT_RESP_OK == SendATCmdWaitResp(str_at, 500, 100, str_ok, 5)) {
					#ifdef DEBUG_ON
			...
			// Neu thu het 8 cai baud ma van khong vo duoc thi ket luan module sim co van de
			#ifdef DEBUG_ON
	...
	// Khong roi vao 2 truong hop tren (tuc la no ko tra loi cai j luon, du da bat nguon)
	if (norepOK==true){
...
#endif
					//HIEP cai nay dung de debug nhan duoc du lieu gi :D
					//Serial.write((char*)comm_buf);
